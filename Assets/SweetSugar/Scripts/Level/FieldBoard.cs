...
namespace SweetSugar.Scripts.Level
 {
     ...
     public class FieldBoard : MonoBehaviour
     {
         ...
         public void CreateField(IColorGenerator generator)
         {
             ...
             foreach (var square in squaresArray)
             {
                 ...
                 for (var sizeRow = 0; sizeRow < squareSubSquare.sizeInSquares.y; sizeRow++)
                 {
                     for (var sizeCol = 0; sizeCol < squareSubSquare.sizeInSquares.x; sizeCol++)
                     {
                         ...
                         {
                             squarePositions.Add(new PositionSquarePair { Row = row1, Col = col1, Square = square });
                         }
                     }
                 }
             }
             ...
             {
                 squaresArray[sq.Row * fieldData.maxCols + sq.Col].subSquares = sq.Square.subSquares;
                 squaresArray[sq.Row * fieldData.maxCols + sq.Col].mainSquareInGrid = sq.Square;
             }
