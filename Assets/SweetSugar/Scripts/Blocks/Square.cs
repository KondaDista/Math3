...
//TODO: Add new target
    ...
    public class Square : MonoBehaviour, ISquareItemCommon, IField, IMarmaladeTargetable
    {
        ...
        public Square mainSquareInGrid;  // main square in grid for big blocks
        ...
        public Item GenItem(bool falling = true, ItemsTypes itemType = ItemsTypes.NONE, int color = -1, ItemForEditor squareBlockItem=null,  bool noAnim = false)
        {
            ...
            if (color == -1)
                itemComponent.GenColor(colorGenerator);
...
//        if (!nextSq?.IsNone() ?? false)
        ...
        private void CreateSubblock(SquareTypes _type, GameObject prefab, int index, SquareTypeLayer layerSquare)
        {
            if (prefab != null && _type != SquareTypes.EmptySquare && _type != SquareTypes.NONE)
            {
                ...
                {
                    var square = field.GetSquare(layerSquare.originalPos).subSquares.FirstOrDefault(i=>i.type==layerSquare.squareType);
                    if (square != null)
                    {
        ...
        public void DestroyBlock(bool destroyTarget=false)
        {
            if (destroyIteration == 0)
            {
                ...
                LevelManager.THIS.delayedCall(0.2f, () => { destroyIteration = 0; });
            }
            else return;
            ...
            if (subSquares.Count > 0)
            {
                ...
                for (int sizeRow = 0; sizeRow < subSquare.sizeInSquares.y; sizeRow++)
                {
                    for (int sizeCol = 0; sizeCol < subSquare.sizeInSquares.x; sizeCol++)
                    {
                        int col1 = mainSquareInGrid.col + sizeCol;
                        int row1 = mainSquareInGrid.row + sizeRow;

                        if (row1 >= field.fieldData.maxRows || col1 >= field.fieldData.maxCols)
                            continue;

                        var neighbourSquare = field.squaresArray[row1 * field.fieldData.maxCols + col1];
                        subSquare = neighbourSquare.UpdateType();
                    }
                }
        ...
        private Square UpdateType()
        {
            ...
            if (subSquare.CanGoInto() && Item == null)
                Item = null;
            return subSquare;
        }
